...
def sentence1() -> Expr:
    ...
    S1 = A | B #S1 = disjoin (A, B)
    S2 = (~A) % ( (~B) | C) #S2 = (~A) % disjoin(~B, C)
    S3 = disjoin((~A), (~B), C) #S3 = disjoin(~A, ~B, C)
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    satisfacible  = (premise & ~conclusion)
    
    return not findModel(satisfacible)
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    direction_sentences = []
    for direction in DIRECTIONS:
        direction_sentences.append(PropSymbolExpr(direction, time=t))
    pacphysics_sentences.append(exactlyOne(direction_sentences))
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    non_outer_wall_coords = list(itertools.product(range(1, width + 1), range(1, height + 1)))
    KB = []
    x0, y0 = x0_y0
    x1, y1 = x1_y1
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        pacman_location = exactlyOne([logic.PropSymbolExpr(pacman_str, x, y, time = t) for x, y in non_wall_coords])
        ...
        for x, y in food:
            ...
            next_food = logic.PropSymbolExpr(food_str, x, y, time = t+1)
            
            KB.append(next_food % (~pacman_loc & food_loc))
...
#____________________________AUX FUNCTIONS__________________________________________________

def Aux1(agent, KB: List, t, all_coords, non_outer_wall_coords, walls_grid, flag) -> List:
    if (flag == 1):
...
def Aux2(KB: List, t, non_outer_wall_coords ) ->Tuple[List, List]:
    possible_locations = []
    cKB = conjoin(KB)

    for x, y in non_outer_wall_coords:
        pacman_loc = PropSymbolExpr(pacman_str, x, y, time = t)
        if entails(cKB, pacman_loc):
            if entails(cKB, ~pacman_loc):
                print ("incongruencia")
            #print("pacman_loc", pacman_loc)
            possible_locations.append((x, y))
            KB.append(pacman_loc)
        elif entails(cKB, ~pacman_loc):
            KB.append(~pacman_loc)
        if findModel(conjoin(KB) & pacman_loc): 
            if (x, y) not in possible_locations:
                possible_locations.append((x, y))
           
    return possible_locations, KB   

def Aux3(agent, KB: List, t, all_coords, non_outer_wall_coords, walls_grid, known_map) -> Tuple[List, List]:
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        KB = Aux1(agent, KB, t, all_coords, non_outer_wall_coords, walls_grid, 1)
        
        possible_locations, KB = Aux2(KB, t, non_outer_wall_coords)
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        KB = Aux1(agent, KB, t, all_coords, non_outer_wall_coords, known_map, 1)
        KB , known_map = Aux3(agent, KB, t, all_coords, non_outer_wall_coords, None, known_map)
        agent.moveToNextState(agent.actions[t])
        "*** END YOUR CODE HERE ***"
        yield known_map
...
def slam(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        KB = Aux1(agent, KB, t, all_coords, non_outer_wall_coords, known_map, 2)
        KB , known_map = Aux3(agent, KB, t, all_coords, non_outer_wall_coords, known_map, known_map)
        possible_locations, KB = Aux2(KB, t, non_outer_wall_coords)
        agent.moveToNextState(agent.actions[t])
