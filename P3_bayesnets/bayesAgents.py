...
def constructHeartAttackBayesNet():
    ...
    SmokesAssignmentDict = {'Smokes' : 'true'}
    SmokesCPT.setProbability(SmokesAssignmentDict, 0.15)
    
    SmokesAssignmentDict = {'Smokes' : 'false'}
    SmokesCPT.setProbability(SmokesAssignmentDict, 0.85)


    BP_CPT  = bn.Factor(['BP'], ['Exercise', 'Smokes'], variableDomainsDict)
    BSE = {'BP' : 'true',  'Smokes' : 'true','Exercise' : 'true'}
    bSE = {'BP' : 'false',  'Smokes' : 'true','Exercise' : 'true'}
    BsE = {'BP' : 'true',  'Smokes' : 'false','Exercise' : 'true'}
    bsE = {'BP' : 'false',  'Smokes' : 'false','Exercise' : 'true'}
    BSe = {'BP' : 'true',  'Smokes' : 'true','Exercise' : 'false'}
    bSe = {'BP' : 'false',  'Smokes' : 'true','Exercise' : 'false'}
    Bse = {'BP' : 'true',  'Smokes' : 'false','Exercise' : 'false'}
    bse = {'BP' : 'false',  'Smokes' : 'false','Exercise' : 'false'}

    BP_CPT.setProbability(BSE, 0.45)
    BP_CPT.setProbability(bSE, 0.55)
    BP_CPT.setProbability(BsE, 0.05)
    BP_CPT.setProbability(bsE, 0.95)
    BP_CPT.setProbability(BSe, 0.95)
    BP_CPT.setProbability(bSe, 0.05)
    BP_CPT.setProbability(Bse, 0.55)
    BP_CPT.setProbability(bse, 0.45)
...
def constructBayesNet(gameState):
    ...
    #edges definition of the known variables
    edges = [(X_POS_VAR, FOOD_HOUSE_VAR), (X_POS_VAR, GHOST_HOUSE_VAR), (Y_POS_VAR, FOOD_HOUSE_VAR), (Y_POS_VAR, GHOST_HOUSE_VAR)]

    #edges definition of the observation variables previously defined at obsVars 
    edges += [(FOOD_HOUSE_VAR, obsVar) for obsVar in obsVars]
    edges += [(GHOST_HOUSE_VAR, obsVar) for obsVar in obsVars]
    
    #definition of the domains of the variables with the constants defined at the top of this file
    ...
    #definition of the domains of the observation variables
...
def fillYCPT(bayesNet, gameState):
    ...
    #definition of the probabilities using the constants defined at the top of this file
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    "*** YOUR CODE HERE ***"
    #inference of the probability distribution of the food house
    #we must consider all posible values for the query variable
    foodHouseMarginal = inference.inferenceByVariableElimination(bayesNet, [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR ], evidence, eliminationOrder)

    #return the most probable location of the food house
    positions = foodHouseMarginal.getAllPossibleAssignmentDicts()
    maxProb = 0
    bestPos = dict()

    for pos in positions:
        foodHouseMarginalprob = foodHouseMarginal.getProbability(pos)
        if foodHouseMarginalprob > maxProb:#comprobamos la probabilidad de cada posiciony si es mayor que la anterior la guardamos
            maxProb = foodHouseMarginalprob
            bestPos = pos
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        first_dictProb = var.getProbability(first_dict)
        second_dictProb = var.getProbability(second_dict)

        leftExpectedValue = first_dictProb * WON_GAME_REWARD + second_dictProb * GHOST_COLLISION_REWARD
        rightExpectedValue = first_dictProb * GHOST_COLLISION_REWARD + second_dictProb * WON_GAME_REWARD"""
